...
namespace Oranikle.Report.Engine
{
	...
	///</summary>
	public class RenderXml: IPresent
	{
		Report r;					// report
...
//				WriteElement(rowstart);
		...
		class ContainerIO
		{
			public StringBuilder attribute_sb;
			public StringWriter attribute_sw;

			public StringBuilder subelement_sb;
			public StringWriter subelement_sw;

			public bool bEmpty=true;

			public ContainerIO(string begin)
			{
			...
			public void WriteElement(string format)
			{
				bEmpty = false;
				subelement_sw.Write(format);
			}

			public void WriteElement(string format, params object[] arg)
			{
				bEmpty = false;
				subelement_sw.Write(format, arg);
			}

			public void WriteElementLine(string format)
			{
				bEmpty = false;
				subelement_sw.WriteLine(format);
			}

			public void WriteElementLine(string format, params object[] arg)
			{
				bEmpty = false;
				subelement_sw.WriteLine(format, arg);
			}

			public void WriteAttribute(string format)
			{
				bEmpty = false;
				attribute_sw.Write(format);
			}

			public void WriteAttribute(string format, params object[] arg)
			{
				bEmpty = false;
				attribute_sw.Write(format, arg);
			}

			public void WriteAttributeLine(string format)
			{
				bEmpty = false;
				attribute_sw.WriteLine(format);
			}

			public void WriteAttributeLine(string format, params object[] arg)
			{
				bEmpty = false;
