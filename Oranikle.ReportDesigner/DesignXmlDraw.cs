	...
	internal class DesignXmlDraw: UserControl
	{
		...
		internal bool DeleteReportItem(XmlNode node)
		{
			...
			if (node.Name == "Table" ||
				...
				node.Name == "List" ||
				node.Name == "Matrix" ||
				node.Name == "System.Drawing.Rectangle")
				bRebuildNames = true;
        ...
        private RectangleF DrawImage(XmlNode xNode, RectangleF r)
        {
            ...
            {	// shouldn't really ever happen
                DrawString("System.Drawing.Image with invalid source or value.", si, ir);
                return ir;
            }
            ...
            switch (sNode.InnerText)
            {
                ...
                case "Embedded":
                    ...
                    break;
                case "Database":
                    DrawString(string.Format("Database System.Drawing.Image: {0}.", vNode.InnerText), si, ir);
                    break;
                default:
                    DrawString(string.Format("System.Drawing.Image, invalid source={0}.", sNode.InnerText), si, ir);
                    break;
            }
            return ir;
        }
		...
		private bool DrawImageEmbedded(XmlNode iNode, XmlNode sNode, XmlNode vNode, StyleInfo si, RectangleF r)
		{
			...
			if (emNode == null)
			{
				DrawString(string.Format("System.Drawing.Image: embedded image {0} requested but there are no embedded images defined.",vNode.InnerText), si, r);
				return false;
			}
			...
			if (eiNode == null)
			{
				DrawString(string.Format("System.Drawing.Image: embedded image {0} not found.",vNode.InnerText), si, r);
				return false;
			}
			...
			{
				DrawString(string.Format("System.Drawing.Image: {0}",e.Message), si, r);
			}
			finally
			{
				if (strm != null)
		...
		private bool DrawImageExternal(XmlNode iNode, XmlNode sNode, XmlNode vNode, StyleInfo si, RectangleF r)
		{
			...
			try 
			{
				...
				{	// System.Drawing.Image is an expression; can't calculate at design time
					DrawString(string.Format("System.Drawing.Image: {0}",vNode.InnerText), si, r);
				}
				else
				{
        ...
        private bool DrawImageSized(XmlNode iNode, System.Drawing.Image im, StyleInfo si, RectangleF r)
        {
            return DrawImageSized(iNode, GetSizing(iNode), im, si, r);
        }

		private bool DrawImageSized(XmlNode iNode, ImageSizingEnum ise, System.Drawing.Image im, StyleInfo si, RectangleF r)
		{
            ...
            switch (ise)
			{
				case ImageSizingEnum.AutoSize:
                    ...
                    ir = new System.Drawing.Rectangle(PixelsX(r2.Left), PixelsY(r2.Top), im.Width, im.Height);
                    g.DrawImage(im, ir);
                    g.PageUnit = gu;
					bResize = true;
					break;
...
//  TODO: this is problematic since it require a PageImage
		...
		internal void PasteImage(XmlNode parent, System.Drawing.Bitmap img, PointF p)
		{
			string t = string.Format(NumberFormatInfo.InvariantInfo, 
				"<ReportItems><System.Drawing.Image><Source>Embedded</Source><Height>{0:0.00}in</Height><Width>{1:0.00}in</Width><Sizing>FitProportional</Sizing></System.Drawing.Image></ReportItems>",
		...
		internal void PasteReportItems(XmlNode parent, string rItems, PointF p)
		{
			...
			{
				XmlNode rectNode = this.CreateElement(riNode, "System.Drawing.Rectangle", null);
		...
		/// </summary>
		/// <param name="r">System.Drawing.Rectangle used to test</param>
		/// <param name="hScroll">current horizontal scroll</param>
		/// <param name="vScroll">current vertical scroll</param>
		internal void SelectInRectangle(System.Drawing.Rectangle r, float hScroll, float vScroll)
		{
			if (rDoc == null)
				return;
		...
		private void SelectInRectangle(XmlNode xNode, RectangleF r)
		{
			...
			SelectInReportItems(ri, rif);
			if (this.SelectedCount == 0)			// if nothing inside selected select System.Drawing.Rectangle itself
				this.AddSelection(xNode);
		}
		...
		{
			return HitRectangle(xNode, r);		// Same logic as System.Drawing.Rectangle
		}
		...
		private HitLocation HitRectangle(XmlNode xNode, RectangleF r)
		{
			...
			hl.HitContainer = xNode;					// System.Drawing.Rectangle is its own container
		...
		private HitLocation HitReportItem(XmlNode xNode, RectangleF r)
		{
			...
			if (rif.Contains(_HitPoint))
			{
				...
				if (bSelected && (xNode.Name == "List" || xNode.Name == "System.Drawing.Rectangle"))
				{
		...
		internal IEnumerable GetReportItems(string query)
		{
			...
			{
				if (query == null)
					query = "//Textbox|//System.Drawing.Rectangle|//System.Drawing.Image|//Subreport|//Chart|//Table|//List|//Line|//Matrix|" +
						"//rdl:Textbox|//rdl:System.Drawing.Rectangle|//rdl:System.Drawing.Image|//rdl:Subreport|//rdl:Chart|//rdl:Table|//rdl:List|//rdl:Line|//rdl:Matrix";
				else
        ...
        internal bool InGrid(XmlNode node)
        {
            for (XmlNode pNode = node.ParentNode; pNode != null; pNode = pNode.ParentNode)
            {
                if (pNode.Name == "fyi:Grid")
                    return true;
                if (pNode.Name == "System.Drawing.Rectangle" || pNode.Name == "Table" || pNode.Name=="Matrix" || pNode.Name=="List")
                    return false;
            }
            return false;
        }
		...
		internal XmlNode TMParent(XmlNode node)
		{
			for (XmlNode pNode = node.ParentNode; pNode != null; pNode = pNode.ParentNode)
			{
				if (pNode.Name == "Table" || pNode.Name == "Matrix" || pNode.Name == "fyi:Grid")
					return pNode;
                if (pNode.Name == "System.Drawing.Rectangle")
                    return null;
			}
			return null;
		}
